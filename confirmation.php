...
if (!isset($_SESSION['email'])) {
      ...
      echo "<script>alert('Lgin Require');document.location='login.php';</script>";
...
$query = 'SELECT products.price, products.id, products.title, products.image, cart.qty, cart.id, cart.product_id from cart, products where products.id = cart.product_id and cart.user_id="' . $user_id . '"';
...
// ,`address_line`,`address_city`,`address_district`,`address_country`
// ,'$add1','$city','$country'
  ...
  $order = "INSERT INTO `orders`(`product_id`, `user_id`, `product_qty`, `order_amount`, `status`,`last_name`,`address_line`,`address_city`,`address_district`,`address_country`,`zip`) 
    VALUES (" . $row['product_id'] . "," . $user_id . "," . $row['qty'] . "," . $row['price'] * $row['qty'] + 49 . ", 'Confirmed', '$name1','$add1','$city','$district','$country','$zip')";
    ...
    <div class="row">
      <div class="col-lg-12">
        <div class="confirmation_tittle">
          ...
          <a href="invoiceproduct.php"><h4 fonr-color:blue;>View Bill.</h4></a>

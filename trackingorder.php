...
$query = $_POST['orderid'];
$orderid = mysqli_real_escape_string($con, $query);
      ...
      <div class="col-lg-12">
        <div class="order_details_iner">
          ...
          <table class="table table-borderless">
            ...
            <tbody>
              <?php
              $allOrders = "SELECT orders.id, orders.order_date ,products.title, orders.product_qty, orders.order_amount, orders.status from orders, products where user_id='$user_id' and orders.product_id=products.id and orders.id='$orderid'";
              ...
              <?php
              $allOrdersgardener = "SELECT orders.id, orders.order_date ,gardener.last_name, orders.product_qty, orders.order_amount, orders.status from orders, gardener where user_id='$user_id' and orders.product_id=gardener.id and orders.id='$orderid'";
              $orderresult2 = mysqli_query($con, $allOrdersgardener);
              while($row = mysqli_fetch_array($orderresult2)){
                echo '<tr>
                        <th><span>'.$row['id'].'</span></th>
                        <th><span>'.$row['order_date'].'</span></th>
                        <th><span>'.$row['last_name'].'</span></th>
                        <th>'.$row['product_qty'].'</th>
                        <th> <span>Rs. '.$row['order_amount'].'</span></th>
                        <th> <span>'.$row['status'].'</span></th>
                      </tr>';
